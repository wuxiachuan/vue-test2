                        ...
                        <el-table :data="wheelList" style="width: 100%" :default-sort = "{prop: 'id', order: 'ascending'}" border stripe>
                            ...
                            <el-table-column prop="" label="完工状态" >
                                <template slot-scope="scope">
                                    <span v-if="scope.row.isbearingrollTestFinish =='0'" >未完工</span>
                                    <span v-if="scope.row.isbearingrollTestFinish =='1'" >完工</span>
                                </template>
                            </el-table-column>
                            <el-table-column label="操作" fixed="right" width="200">
                                <template slot-scope="scope">
                                    ...
                                    <el-button
                                            ...
                                            size="mini"
                                            type="danger"
                                            :disabled="scope.row.isbearingrollTestFinish =='0'"
            ...
            <el-card>
                ...
                <el-form :model="ruleForm"  ref="ruleForm" label-width="100px" :disabled="disableForm">
                    ...
                    <el-form-item>
                        ...
                        <el-button @click="modifyForm('ruleForm')" :disabled="this.wheelInfo.isprocessFinish=='1'" disabled v-show="modbtnstatus" size="small">修改</el-button>
        ...
        methods: {
                    ...
                    if (valid) {
                        ...
                        for(var i=0;i<this.wheelList.length;i++){
                            if (this.wheelList[i].wheelId == this.ruleForm.wheelId){
                                this.wheelInfo.isbearingrollTestFinish = '1';
            ...
            async getSavedWheelInfoFromSession(){
                ...
                var result = await this.$http.get(
                    "/bearingTest/getSavedWheelInfo?name="+name);
                if (result.data.code != 100){
                    alert("查询失败");
                    return ;
                }
            ...
            async searchUnFinish(){
                this.issaved = '0';
                var result = await this.$http.post(
                    "/bearingTest/unFinishBearingTest2",
                    this.ruleForm);
                if (result.data.code != 100){
                    alert("查询失败");
                    return ;
            ...
            searchWheelInfo(searchForm){
                ...
                this.$refs[searchForm].validate(async (valid) => {
                    if (valid) {
                        var result = await this.$http.post(
                            "/bearingTest/searchWheelInfoByconditionTest",
                            this.search);
                        if (result.data.code != 100){
                            alert("添加失败");
                            return ;
            ...
            async showitem(index,item){
                ...
                if (item.isbearingrollTestFinish == '1'){
                    ...
                    //已完成从数据库内查找，提交到显示表单
                    var result = await this.$http.get(
                        "/bearingTest/findBearingTestById?id="+id);
                    if (result.data.code != 100){
                        alert("添加失败");
                        return ;
                    }
            ...
            async handleProblemTable(done){
                if(this.wheelInfo.isbearingrollTestFinish=='0'&&this.hassaved == false){
            ...
            async deleteForm(index,item){
                ...
                if (res == "confirm") {
                    ...
                    var result = await this.$http.get('/bearingTest/deleteBearingTest?id=' + id);
                    if (result.data.code != 100) {
                        alert("删除失败");
                        return;
                    }
