                        ...
                        <el-table :data="wheelList" style="width: 100%" :default-sort = "{prop: 'id', order: 'ascending'}" border stripe>
                            ...
                            <el-table-column prop="" label="完工状态" >
                                <template slot-scope="scope">
                                    <span v-if="scope.row.isbearingCapFinish =='0'" >未完工</span>
                                    <span v-if="scope.row.isbearingCapFinish =='1'" >完工</span>
                                </template>
                            </el-table-column>
                            <el-table-column label="操作" fixed="right" width="200">
                                <template slot-scope="scope">
                                    ...
                                    <el-button
                                            ...
                                            size="mini"
                                            type="danger"
                                            :disabled="scope.row.isbearingCapFinish =='0'"
        ...
        methods: {
                    ...
                    if (valid) {
                        ...
                        for(var i=0;i<this.wheelList.length;i++){
                            if (this.wheelList[i].wheelId == this.ruleForm.wheelId){
                                this.wheelInfo.isbearingCapFinish = '1';
            ...
            async getSavedWheelInfoFromSession(){
                ...
                var result = await this.$http.get(
                    "/bearingCap/getSavedWheelInfo?name="+name);
                if (result.data.code != 100){
                    alert("查询失败");
                    return ;
                }
            ...
            async searchUnFinish(){
                this.issaved = '0';
                var result = await this.$http.post(
                    "/bearingCap/unFinishBearingCap2",
                    this.ruleForm);
                if (result.data.code != 100){
                    alert("查询失败");
                    return ;
            ...
            async showitem(index,item){
                ...
                if (item.isbearingCapFinish == '1'){
                    ...
                    //已完成从数据库内查找，提交到显示表单
                    var result = await this.$http.get(
                        "/bearingCap/findBearingCapById?id="+id);
                    if (result.data.code != 100){
                        alert("添加失败");
                        return ;
                    }
            ...
            async handleProblemTable(done){
                if(this.wheelInfo.isbearingCapFinish=='0'&&this.hassaved == false){
