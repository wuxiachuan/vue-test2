                        ...
                        <el-table :data="wheelList" style="width: 100%" :default-sort = "{prop: 'id', order: 'ascending'}" border stripe>
                            ...
                            <el-table-column prop="" label="完工状态" >
                                <template slot-scope="scope">
                                    <span v-if="scope.row.ismagnetInspectionFinish =='0'" >未完工</span>
                                    <span v-if="scope.row.ismagnetInspectionFinish =='1'" >完工</span>
                                </template>
                            </el-table-column>
                            <el-table-column label="操作" fixed="right" width="200">
                                <template slot-scope="scope">
                                    ...
                                    <el-button
                                            ...
                                            size="mini"
                                            type="danger"
                                            :disabled="scope.row.ismagnetInspectionFinish =='0'"
    ...
    export default {
        ...
        methods: {
                    ...
                    if (valid) {
                        ...
                        for(var i=0;i<this.wheelList.length;i++){
                            if (this.wheelList[i].wheelId == this.ruleForm.wheelId){
                                this.wheelInfo.ismagnetInspectionFinish = '1';
            ...
            async saveToSession(info,index,heads){
                ...
                var result = await this.$http.post(
                    "/axleInspection/savedWheelInfoM",
                    {name:name,data:data});
                if (result.data.code != 100){
                    alert("查询失败");
                    return ;
            ...
            async getSavedWheelInfoFromSession(){
                ...
                var result = await this.$http.get(
                    "/axleInspection/getSavedWheelInfoM?name="+name);
                if (result.data.code != 100){
                    alert("查询失败");
                    return ;
                }
            ...
            async searchUnFinish(){
                this.issaved = '0';
                var result = await this.$http.post(
                    "/axleInspection/unFinishMagInspection2",
                    this.ruleForm);
                if (result.data.code != 100){
                    alert("查询失败");
                    return ;
            ...
            async captureitem(index,item){
                ...
                //未完成给个空值和表单id
                if (item.ismagnetInspectionFinish == '0'){
                    //从候选set中删除
                    var result = await this.$http.get(
                        "/axleInspection/chooseWheelM?id="+id);
                    if (result.data.code != 100){
                        //获取未完成列表
                        this.searchUnFinish();
                        alert("捕获失败");
            ...
            async showitem(index,item){
                ...
                //是否完成
                if (item.ismagnetInspectionFinish == '1'){
                    ...
                    //已完成从数据库内查找，提交到显示表单
                    var result = await this.$http.get(
                        "/axleInspection/findMagInspectionById?id="+id);
                    if (result.data.code != 100){
                        alert("添加失败");
                        return ;
                    }
            ...
            async turnBack(index,item){
                const id = item.wheelId;
                var result = await this.$http.get(
                    "/axleInspection/turnBackM?id="+id);
                if (result.data.code != 100){
                    alert("添加失败");
                    return ;
                }
            ...
            async handleProblemTable(done){
                if(this.wheelInfo.ismagnetInspectionFinish=='0'&&this.hassaved == false){
