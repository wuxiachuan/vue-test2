                ...
                <el-col :span="7">
                    <el-card class="wheelInfo-Container">
                        ...
                        <ul class="wheelInfo-list" ref="infolist">
                            <li :class="['wheelInfo-list-item',item.ismagnetInspectionFinish=='0'?'notfinish':'']" v-for="(item,index) in wheelList" :ref="'li'+index"
    ...
    export default {
        ...
        methods: {
            ...
            saveToSession(data,index,heads){
                ...
                sessionStorage.setItem("savedMagInspection",JSON.stringify(wheelInfo));
            },
            //从seesion中获取保存的未完成表单
            getSavedWheelInfoFromSession(){
                var measureInfo = JSON.parse(sessionStorage.getItem("savedMagInspection"));
                if (measureInfo!=null){
