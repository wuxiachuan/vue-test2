                        ...
                        <el-table :data="wheelList" style="width: 100%" :default-sort = "{prop: 'id', order: 'ascending'}" border stripe>
                            ...
                            <el-table-column prop="" label="完工状态" >
                                <template slot-scope="scope">
                                    <span v-if="scope.row.isMeasureFinish =='0'" >未完工</span>
                                    <span v-if="scope.row.isMeasureFinish =='1'" >完工</span>
                                </template>
                            </el-table-column>
                            <el-table-column label="操作" fixed="right" width="200">
                                <template slot-scope="scope">
                                    ...
                                    <el-button
                                            ...
                                            size="mini"
                                            type="danger"
                                            :disabled="scope.row.isMeasureFinish =='0'"
        ...
        methods: {
            ...
            searchWheelInfo(searchForm){
                ...
                this.$refs[searchForm].validate(async (valid) => {
                    if (valid) {
                        var result = await this.$http.post(
                            "/wheelTakein/searchWheelInfoByconditionMeasure",
                            this.search);
                        if (result.data.code != 100){
                            alert("添加失败");
                            return ;
            ...
            async showitem(index,item){
                ...
                if (item.isMeasureFinish == '1'){
                    ...
                    //已完成从数据库内查找，提交到显示表单
                    var result = await this.$http.get(
                        "/wheelTakein/findMeasureById?id="+id);
                    if (result.data.code != 100){
                        alert("添加失败");
                        return ;
                    }
            ...
            async deleteForm(index,item){
                ...
                if (res == "confirm") {
                    ...
                    var result = await this.$http.get('/wheelTakein/deleteMeasure?id=' + id);
                    if (result.data.code != 100) {
                        alert("删除失败");
                        return;
                    }
