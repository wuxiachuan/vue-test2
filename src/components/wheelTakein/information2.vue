...
<!--                        <el-button  @click="searchSavedInfo" size="small">已保存</el-button>-->
                        <el-button @click="creatNewWheelInfo" >新建</el-button>
<!--                        <el-button  @click="creatQrcode" size="small">二维码</el-button>-->
                        ...
                        <el-table :data="wheelList" style="width: 100%" :default-sort = "{prop: 'id', order: 'ascending'}" border stripe>
                            ...
                            <el-table-column prop="" label="完工状态" >
                                <template slot-scope="scope">
                                    <span v-if="scope.row.infoTakeFinish =='0'" >未完工</span>
                                    <span v-if="scope.row.infoTakeFinish =='1'" >完工</span>
                                </template>
                            </el-table-column>
                            <el-table-column label="操作" fixed="right" width="280">
                                <template slot-scope="scope">
                                    ...
                                    <el-button
                                            icon="el-icon-edit"
                                            size="mini"
                                            @click="creatQrcode(scope.$index, scope.row)">二维码</el-button>
                                    <el-button
                                            icon="el-icon-delete"
                                            size="mini"
                                            type="danger"
        ...
        >
            <el-card>
                ...
                <el-form :model="ruleForm" :rules="rrules" ref="ruleForm" label-width="100px" :disabled="disableForm">
                    <el-row>
                        <el-col :span="8">
        ...
        methods: {
            ...
            async getSavedWheelInfoFromSession(){
                ...
                var result = await this.$http.get(
                    "/wheelTakein/getSavedWheelInfo?name="+name);
                if (result.data.code != 100){
                    alert("查询失败");
                    return ;
                }
            ...
            async modifyForm(formName){
                ...
                //轴承检查 0 未完成，1已完成
                this.ruleForm.isbearingRepairFinish= "0";
                //超声波探伤 -1不需要，0未完成，1已完成
                this.ruleForm.isaxleInspectionFinish= "-1";
                //车轮旋面 -1不需要，0未完成，1已完成
                this.ruleForm.isWheelRoundingFinish= "-1";
                //轴承压装 -1不需要，1左端压装，2右端压装，3两端压装，4已完成
                this.ruleForm.isbearingLoadFinish= "-1";
                //磁粉探伤 -1不需要，0未完成，1已完成
                this.ruleForm.ismagnetInspectionFinish = "-1";
                this.ruleForm.isbearingCapFinish= "-1";
                //旋修关盖 -1不需要，0未完成，1已完成
                this.ruleForm.isbearingCapFinishW= "-1";
                //推卸关盖 -1不需要，0未完成，1已完成
                this.ruleForm.isbearingCapFinishL= "-1";
                //磨合 0 未完成，1已完成
                this.ruleForm.isbearingrollTestFinish= "0";
                //支出 0 未完成，1已完成
                this.ruleForm.isreMeasureFinish = "0";
                //支出 0 未完成，1已完成
                this.ruleForm.iswheelDispatchFinish= "0";
                //质检 0 未完成，1已完成
                this.ruleForm.isqualityInspectionFinish= "0";
                //验收 0 未完成，1已完成
                this.ruleForm.isverifyFinish= "0";
                //是否完工 0 未完成，1已完成
                this.ruleForm.isprocessFinish= "0";
            ...
            async handleProblemTable(done){
                if(this.wheelInfo.infoTakeFinish=='0'&&this.hassaved == false){
            ...
            async deleteForm(index,item){
                ...
                if (res == "confirm") {
                    ...
                    var result = await this.$http.get('/wheelTakein/delete?id=' + id);
                    if (result.data.code != 100) {
                        alert("删除失败");
                        return;
                    }
